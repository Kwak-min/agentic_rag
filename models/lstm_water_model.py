# models/lstm_water_model.py - 수위 예측 LSTM 모델
...
class WaterLevelLSTM:
    ...
    def __init__(self,
        ...
        logger.info(f"LSTM 모델 초기화: seq_length={seq_length}, pred_length={pred_length}")
    ...
    def build_model(self, input_shape: Tuple[int, int]) -> keras.Model:
        ...
        self.model = model
        logger.info(f"모델 구축 완료: {model.count_params():,}개 파라미터")
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray) -> Dict:
        ...
        logger.info(f"평가 결과: MSE={mse:.6f}, RMSE={rmse:.6f}, MAE={mae:.6f}, MAPE={mape:.2f}%")
        return metrics
    ...
    def predict_future(self,
        ...
        for _ in range(n_steps):
            # 예측 (1 step)
            pred = self.model.predict(current_seq.reshape(1, -1, current_seq.shape[-1]), verbose=0)
...
class AnomalyDetector:
    ...
    def __init__(self,
        ...
        logger.info(f"이상 탐지기 초기화: threshold_multiplier={threshold_multiplier}")
    ...
    def fit_threshold(self, X_normal: np.ndarray, y_normal: np.ndarray):
        ...
        """
        # 정상 데이터 예측
        y_pred = self.lstm_model.predict(X_normal)
        ...
        logger.info(f"임계값 설정: mean={self.mean_error:.6f}, std={self.std_error:.6f}, threshold={self.threshold:.6f}")
